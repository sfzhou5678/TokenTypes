...
def str_node(node):
    ...
    if isinstance(node, ast.operator):
        ...
        return 'Arg(\'%s\')' % node.arg
    elif isinstance(node, ast.alias):
        return 'AsName(\'%s\')'% node.name
    elif isinstance(node, ast.Global):
        rv = '%s { %s }' % (node.__class__.__name__, ' '.join('Name(\'%s\')' % name for name in node.names))
        return rv
    ...
    # 函数、类定义
        ...
        fields = [str_node(val) for field, val in ast.iter_fields(node) if field not in ('name')]
        fields.insert(0, 'ClassName(\'%s\')'%node.name)
    elif isinstance(node, ast.FunctionDef):
        fields = [str_node(val) for field, val in ast.iter_fields(node) if field in ('args', 'body', 'returns')]
        fields.insert(0, 'FuncName(\'%s\')' % node.name)
    elif isinstance(node, ast.AsyncFunctionDef):
        fields = [str_node(val) for field, val in ast.iter_fields(node) if field in ('args', 'body', 'returns')]
        fields.insert(0, 'AsyncFuncName(\'%s\')' % node.name)
